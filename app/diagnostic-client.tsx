...
export default function DiagnosticClient() {
  ...
  if (loadError) return <main className="site-shell grid min-h-screen place-items-center px-5"><Card className="w-full max-w-xl border-0 bg-white/90 shadow-xl"><CardContent className="space-y-5 text-center"><div className="mx-auto grid size-14 place-items-center rounded-2xl bg-orange-50 text-[var(--orange)]"><RotateCcw /></div><h1 className="text-2xl font-bold">Le diagnostic est momentanément indisponible.</h1><p className="text-[var(--muted-foreground)]">{loadError}</p><Button onClick={() => window.location.reload()}>Réessayer</Button></CardContent></Card></main>;
  if (!definition) return <main className="site-shell grid min-h-screen place-items-center"><div className="text-center"><LoaderCircle className="mx-auto mb-4 size-8 animate-spin text-[var(--teal)]" /><p className="font-semibold">Préparation de ton diagnostic…</p></div></main>;

  return <main className="site-shell min-h-screen">
    <header className="topbar"><div className="mx-auto flex max-w-[1380px] items-center justify-between px-5 py-4 sm:px-8"><div className="brand-lockup"><div className="brand-mark">α</div><div><p className="brand-name">alphaducation</p><p className="brand-subtitle">Diagnostic pédagogique</p></div></div><div className="hidden items-center gap-2 rounded-full border border-[var(--line)] bg-white/70 px-4 py-2 text-sm font-semibold text-[var(--deep-blue)] sm:flex"><Clock3 className="size-4" /> environ {definition.estimated_minutes} min</div></div><Progress value={progress} className="h-1 rounded-none bg-transparent [&_[data-slot=progress-indicator]]:bg-[var(--orange)]" /></header>
    <div className="mx-auto grid max-w-[1380px] gap-8 px-5 py-8 sm:px-8 lg:grid-cols-[250px_minmax(0,1fr)] lg:py-12">
      <aside className="hidden lg:block"><div className="sticky top-28 space-y-8"><nav aria-label="Étapes du diagnostic" className="step-nav">{phases.map((item, i) => { const done = i < activePhase; const active = item.key === phase; return <div key={item.key} className={`step-item ${active ? "is-active" : ""} ${done ? "is-done" : ""}`}><span className="step-number">{done ? <Check className="size-4" /> : item.short}</span><span>{item.label}</span></div>; })}</nav><div className="privacy-note"><ShieldCheck className="size-5 text-[var(--teal)]" /><p><strong>Données protégées</strong><br />Tes réponses détaillées restent confidentielles et servent uniquement au suivi pédagogique.</p></div></div></aside>
      <section className="min-w-0"><div className="mb-5 flex items-center justify-between lg:hidden"><p className="text-sm font-bold text-[var(--deep-blue)]">{phases[activePhase]?.label}</p><p className="text-sm text-[var(--muted-foreground)]">{progress}%</p></div>

      {phase === "identity" && <div className="content-stack"><div className="intro-grid"><div><p className="eyebrow">{definition.content.intro.eyebrow}</p><h1 className="display-title">{definition.content.intro.title}</h1><p className="lead">{definition.content.intro.description}</p><div className="feature-strip"><span><Target /> 12 défis mathématiques</span><span><BrainCircuit /> Méthode & mentalité</span><span><ClipboardCheck /> Plan personnalisé</span></div></div><div className="intro-seal" aria-hidden="true"><span className="seal-orbit orbit-one" /><span className="seal-orbit orbit-two" /><span className="seal-core"><Sparkles /></span></div></div>
        <Card className="form-card"><CardContent className="space-y-7"><div><p className="section-kicker">Avant de commencer</p><h2 className="section-title">Qui réalise le diagnostic ?</h2><p className="mt-2 text-[var(--muted-foreground)]">Ces informations permettront à Vincent de relier le bilan au bon élève.</p></div><div className="grid gap-5 sm:grid-cols-2">
