...
function AttemptDetailPanel({ attempt, loading, onClose, onSaved }: { attempt: AttemptDetail | null; loading: boolean; onClose: () => void; onSaved: () => void }) {
  ...
  async function saveReview() {
    ...
    try {
      const response = await fetch(`/api/admin/attempts/${attempt.id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({
        status,
        notes,
        professionalSummary: attempt.review.professionalSummary ?? "",
        studyPlan: attempt.review.studyPlan ?? [],
      }) });
      const data = await response.json();
...
export default function AdminDashboard() {
  ...
  return <main className="admin-shell min-h-screen">
    ...
    <div className="admin-layout">
      <section className="admin-heading"><div><p className="section-kicker">AlphaDiagnostic · Multi-niveaux</p><h1>Suivi des élèves</h1><p>Repère les profils prioritaires, relis les résultats et prépare chaque entretien.</p></div><Button variant="outline" onClick={loadAttempts} disabled={loading}><RefreshCw className={loading ? "animate-spin" : ""} /> Actualiser</Button></section>
      ...
      <section className="admin-workspace">
        <div className="admin-toolbar"><div className="admin-search"><Search /><Input aria-label="Rechercher un élève" placeholder="Rechercher un élève ou un contact…" value={search} onChange={(event) => setSearch(event.target.value)} /></div><NativeSelect aria-label="Filtrer par niveau" value={grade} onChange={(event) => setGrade(event.target.value)}><NativeSelectOption value="">Tous les niveaux</NativeSelectOption><NativeSelectOption value="G6">Grade 6 / EB6</NativeSelectOption><NativeSelectOption value="G7">Grade 7 / EB7</NativeSelectOption><NativeSelectOption value="G8">Grade 8 / EB8</NativeSelectOption><NativeSelectOption value="G9">Grade 9 / EB9</NativeSelectOption><NativeSelectOption value="S1">Secondaire 1</NativeSelectOption><NativeSelectOption value="S2 SCI">Secondaire 2 · SCI</NativeSelectOption><NativeSelectOption value="S2 HUM">Secondaire 2 · HUM</NativeSelectOption><NativeSelectOption value="T SG">Terminale · SG</NativeSelectOption><NativeSelectOption value="T SV">Terminale · SV</NativeSelectOption><NativeSelectOption value="T SE">Terminale · SE</NativeSelectOption><NativeSelectOption value="T LH">Terminale · LH</NativeSelectOption><NativeSelectOption value="EB6">Ancien EB6</NativeSelectOption><NativeSelectOption value="Entrée en EB7">Ancienne entrée EB7</NativeSelectOption><NativeSelectOption value="EB7">Ancien EB7</NativeSelectOption></NativeSelect><NativeSelect aria-label="Filtrer par statut" value={status} onChange={(event) => setStatus(event.target.value)}><NativeSelectOption value="">Tous les statuts</NativeSelectOption><NativeSelectOption value="new">Nouveau</NativeSelectOption><NativeSelectOption value="in_review">En cours</NativeSelectOption><NativeSelectOption value="reviewed">Relu</NativeSelectOption></NativeSelect></div>
