...
create table if not exists private.diagnostic_attempt_reviews (
  ...
  professional_summary text not null default '' check (char_length(professional_summary) <= 6000),
  study_plan jsonb not null default '[]'::jsonb check (jsonb_typeof(study_plan) = 'array' and jsonb_array_length(study_plan) <= 14),
